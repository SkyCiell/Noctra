...
export const LyricsOverlay: React.FC<LyricsOverlayProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 md:p-10 select-none animate-fadeIn">
      ...
      <div className="relative z-10 flex h-[88vh] sm:h-[85vh] w-full max-w-[95vw] sm:max-w-2xl flex-col rounded-2xl sm:rounded-3xl bg-white/[0.04] p-4 sm:p-8 backdrop-blur-3xl border border-white/15 shadow-[0_0_60px_rgba(0,0,0,0.8)] overflow-hidden">
        {/* Header */}
        <div className="flex items-center justify-between border-b border-white/10 pb-3 sm:pb-4 mb-3 sm:mb-4">
          <div className="flex items-center gap-2.5 sm:gap-3 min-w-0 flex-1 mr-2">
            <div className="relative flex h-10 w-10 sm:h-11 sm:w-11 shrink-0 items-center justify-center rounded-xl sm:rounded-2xl overflow-hidden bg-indigo-500/20 text-indigo-400 border border-white/15 shadow-md">
              {song.cover ? (
        ...
        <div
          ref={containerRef}
          className="flex-1 overflow-y-auto space-y-4 sm:space-y-5 px-2 py-6 sm:py-8 text-center scrollbar-none"
        >
          {isLoading ? (
            ...
            <div className="flex h-full flex-col items-center justify-center text-white/40">
              <Music className="h-8 w-8 sm:h-10 sm:w-10 mb-2 opacity-50" />
              <p className="text-xs sm:text-sm">Instrumental soundscape</p>
            </div>
          ) : (
              ...
              return (
                <button
                  ...
                  className={`group block w-full text-center transition-all duration-500 focus:outline-none ${
                    isActive
                      ? 'scale-105 font-bold text-white drop-shadow-[0_0_25px_rgba(255,255,255,0.8)] py-2 sm:py-3'
                      : isPast
                      ? 'text-white/30 hover:text-white/60 font-light text-sm sm:text-base'
                      : 'text-white/50 hover:text-white/80 font-light text-sm sm:text-base'
                  }`}
                >
                  <p
                    className={`transition-all duration-300 ${
                      isActive ? 'text-lg sm:text-2xl md:text-3xl' : ''
                    }`}
                    style={{
